...
select Warehouse_block, sum(Weight_in_kg) as weight_load from Shipping
group by Warehouse_block
order by weight_load;

-- 2. What is the total cost of production with reference to Product importance?

select Product_Importance, sum(Cost_of_the_Product) as production_cost, count(*) as product_count from Shipping
group by Product_Importance
order by production_cost desc;

-- 3. What is the sum of product that arrived On-time?

select count(*) as On_time_deliveries from Shipping
where Reached_on_Time_Y_N = 'On time'

-- 4. How many male customers get discounts above 30?

select top 1 Gender, count(*) as mid_level_discounts from Shipping
where Discount_offered > 30
group by Gender;

-- 5. What is the average weight by Product importance?

select Product_Importance, avg(Weight_in_kg) as average_weight from Shipping
group by Product_Importance
order by average_weight desc;

-- 6. Which mode of shipment is the least used by male?

select top 1 Gender, Mode_of_Shipment, count(*) as customer_count from Shipping
where Gender = 'Male'
group by Gender, Mode_of_Shipment
order by customer_count asc;

-- 7. Which mode of shipment is the least used by female?
...
select Gender, count(*) as gender_distribution from Shipping
group by Gender;
